  ...
  async draw() {
    ...
    const {
      ...
      if (canvasWrapper.firstElementChild?.tagName === "CANVAS") {
        ...
        // Add overlay div on top of the canvas
    ...
    overlay.style.setProperty("--theme-color", "transparent");
    overlay.style.setProperty("--theme-opacity", "0");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  themeSelect.addEventListener("change", () => {
    localStorage.setItem("nightInkTheme", themeSelect.value);
    applyTheme();
  });

  // 🔁 Brightness change
  brightnessSlider.addEventListener("input", applyTheme);

  // 🧠 Restore theme on load
  const saved = localStorage.getItem("nightInkTheme");
  if (saved) {
